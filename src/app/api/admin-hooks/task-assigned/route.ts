...
import { sendTaskAssignedEmail } from '@/lib/emails/send'
...
// Triggered by the admin app when an assessment task is assigned.
// The portal owns email delivery so the email can use the same branding,
// logo attachment, and notification logging as the rest of the applicant flow.
export async function POST(req: NextRequest) {
  ...
  const [{ data: application, error: appError }, { data: task, error: taskError }] = await Promise.all([
    ...
    supabase
      ...
      .select('title, title_ar, subtitle_ar, context_ar, requirements_mid, requirements_senior, deliverables, evaluation_criteria')
      .eq('id', taskId)
      .single(),
  ])
